    ...
    const fetchPost = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_BACKEND_URL}/api/getPostWithId/${postId}`
        );
  ...
  return (
    <div>
      <Title>
        {post.poster} on
        <br />
        {format(new Date(post.date), "MMMM do 'at' HH:mm")}
      </Title>
      ...
      <InfoContainer>
        ...
        <div>
          What was good about today? <br />
          Name 3 things, and send them to strangers, or to a friend.
        </div>
      </InfoContainer>
      ...
      <div className="buttonContainer">
        <Button>
          <NavLink to="/newpost">
            <img
              className="writeImg"
              src={birbImages.writebutton}
              alt="hi"
            ></img>
            <p>
              send your own 3 little Birbs out into the wild or to a friend of
              yours
            </p>
          </NavLink>
        </Button>
